...
namespace PackageManagement.AppSyndication.Navigation {
    ...
    internal class Feed : Swid {
        ...
        internal Feed(Request request, Swidtag swidtag)
            : base(request, swidtag) {
        }

        internal Feed(Request request, IEnumerable<Link> mirrors)
            : base(request, mirrors) {
        }

        internal Feed(Request request, IEnumerable<Uri> mirrors)
            : base(request, mirrors) {
        }
        ...
        internal IEnumerable<Package> Query(string name, string version) {
            ...
            var packages = PackagesFilteredByName(name).Select(packageGroup => new Package(_request, packageGroup))
                .Where(package => package.IsValid && package.Name.EqualsIgnoreCase(name) && VersionComparer.CompareVersions(package.VersionScheme, package.Version, version) == 0);
            ...
            var packagesByName = Feeds.Where(feedGroup => feedGroup.Any(link => {
                if (name.EqualsIgnoreCase(link.Attributes[Iso19770_2.Discovery.Name])) {
                    ...
                    if (!string.IsNullOrEmpty(minVer)) {
                        ...
                        if (VersionComparer.CompareVersions(Iso19770_2.VersionScheme.Unknown, minVer, version) > 0) {
                    ...
                    if (!string.IsNullOrEmpty(maxVer)) {
                        // since we don't know the version scheme at this point, so we just have to guess.
                        if (VersionComparer.CompareVersions(Iso19770_2.VersionScheme.Unknown, version, maxVer) > 0) {
        ...
        internal IEnumerable<Package> Query(string name, string minimumVersion, string maximumVersion) {
            ...
            var packages = PackagesFilteredByName(name).Select(packageGroup => new Package(_request, packageGroup)).Where(package => {
                if (package.IsValid && package.Name.EqualsIgnoreCase(name)) {
                    if (!string.IsNullOrWhiteSpace(minimumVersion)) {
                        if (VersionComparer.CompareVersions(package.VersionScheme, package.Version, minimumVersion) < 0) {
                    ...
                    if (!string.IsNullOrWhiteSpace(maximumVersion)) {
                        if (VersionComparer.CompareVersions(package.VersionScheme, package.Version, maximumVersion) > 0) {
            ...
            var packagesByName = Feeds.Where(feedGroup => feedGroup.Any(link => {
                if (name.EqualsIgnoreCase(link.Attributes[Iso19770_2.Discovery.Name])) {
                    ...
                    if (!string.IsNullOrEmpty(maxVer)) {
                        // since we don't know the version scheme at this point, so we just have to guess.
                        if (VersionComparer.CompareVersions(Iso19770_2.VersionScheme.Unknown, minimumVersion, maxVer) <= 0) {
                    ...
                    if (!string.IsNullOrEmpty(minVer)) {
                        // since we don't know the version scheme at this point, so we just have to guess.
                        if (VersionComparer.CompareVersions(Iso19770_2.VersionScheme.Unknown, maximumVersion, minVer) >= 0) {
